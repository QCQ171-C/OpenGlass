...
- Keep the injected DLL's registry lookup independent from the GUI's recommended write-location policy. Every named runtime value accepts `HKCU → HKLM → default`; for an overridable colorization property, each hive's Override name precedes its base name, producing `HKCU Override → HKCU base → HKLM Override → HKLM base → default`. This is a long-term compatibility contract for transformation packs and plug-ins, not a temporary migration fallback. The GUI's HKCU/HKLM placement is a product convention, not a runtime validity rule. Recommended-location enforcement applies only to writes by the official GUI and preset packages. The GUI scans for values outside its recommended location on every startup, stays silent when none exist, and performs a transactional two-hive migration only after confirmation.
...
`OpenGlass/ProjectionSchemas/legacy/` and `milcomp/` are the only editable projection inventories. Each architecture has independent schema-version-4 `udwm.json` and `dwmcore.json` files containing typed logical Symbols, per-version resolution `bindings`, projected bindings, exact `UNDNAME_COMPLETE` symbol names, Layout cases, fallbacks, and audit notes. Every logical Symbol owns one typed slot and a non-empty ordered `bindings` array; each binding owns its exact names and one `[min_inclusive, max_exclusive)` interval. Binding intervals within a Symbol must not overlap, and an uncovered interval makes that Symbol version-inactive. Use `notes` only for non-obvious reverse-engineering guidance such as semantic anchors, constructor or xref routes, adjusted-`this`, ABI traps, ICF/inlining ambiguity, and independent cross-checks. Do not duplicate facts already expressed by binding names/ranges or ordinary consumer references, and never replace useful guidance with generic migration or PDB provenance. `Scripts/projection_codegen.py` requires `--architecture legacy|milcomp` and emits generated C++ only under the selected project's `$(IntDir)`. The compatibility `*.Offsets.hpp` headers only include generated declarations; they are not a second data source.
...
Generated `SymbolHandle` and `FieldHandle` values encode only module/index/type. Required Symbols are a global startup prerequisite only when their current-version binding is active. Both modules follow `Reset → Collect → Validate → Commit`, and neither publishes slots or variables unless both validate. A logical Symbol may have multiple non-overlapping bindings but still owns one resolution state and one slot; inactive gaps do not participate in Required validation. Optional or version-inactive projected functions must have an ABI-compatible fallback; otherwise inactive calls use the typed FailFast thunk. Do not introduce component feature masks or partial readiness.

Raw Symbols must declare their real function-pointer ABI. Use `BYTE*` with `usage: "code_address"` only for instruction-pattern navigation that intentionally treats a symbol as bytes. Merge versioned entries into one logical Symbol only when module, semantic target, kind/usage, requirement, fallback, and underlying ABI are identical; express their exact names and disjoint intervals as separate bindings. If an ABI changes across builds, keep separate typed logical Symbols even when their ranges are mutually exclusive. The exceptional `abi_compatibility` forms (`discard_return` and `extra_trailing_argument`) require an explicit source type and are compile-time checked; never hide an ABI change by placing incompatible complete names in one logical Symbol.

An exact function-pointer ABI does not by itself prove that an internal DWM function is safe to detour through an ordinary C++ dispatcher. Whole-program optimization may let a caller rely on a known callee, or a short wrapper around it, preserving volatile GPRs, XMM lanes, or EFLAGS beyond the public Windows x64 ABI. Audit every hooked function on the exact binary by following all direct callers and intervening wrappers, and check values live across the call before approving a normal `Detour` or `ChainDetour`. Treat register allocation and instruction patterns as candidate evidence only; prove the downstream semantic use and eliminate infeasible paths and value-independent idioms. When a private contract is verified, preserve it in the physical dispatcher with the narrowest version-specific `CustomDispatchDetour`/assembly shim, document the decisive call chain, and keep replacement dispatch plus `HookRundown` inside the preserved region. Do not generalize one confirmed contract to other functions or builds, and do not misrepresent it as a projected signature change.
...
python Scripts/audit_legacy_live_preview_highlight.py . --samples-root OGDcomplie --symbol-path Cache\symbol-catalog\symbols --dbghelp PATH_TO_DBGHELP
